...
TRACE="$ROOT/compat/vendor-usbmon.log"
RUN_LOG="$ROOT/compat/vendor-open.log"
...
: >"$TRACE"
: >"$RUN_LOG"
timeout 8 cat "/sys/kernel/debug/usb/usbmon/${bus}u" >"$TRACE" &
monitor_pid=$!
sleep 0.2
...
env \
  ...
  timeout 6 "$ROOT/compat/vendor-open" >"$RUN_LOG" 2>&1 || true
